...
void	ft_check_map_values(t_game *game)
{
	if(game->map[game->move_x][game->move_y] == 'C')
	{
		...
		mlx_image_to_window(game->mlx, game->floor, game->move_y * 32,  game->move_x * 32);
	...
	if (game->map[game->move_x][game->move_y] == 'E' && game->count_collectables == 0)
		ft_open_exit(game);
	if (game->map[game->move_x][game->move_y] == 'E' && game->count_collectables != 0)
...
void	ft_player_right(t_game *game)
{
	...
	if (!game->player)
		ft_print_error_msg("PLAYER right problem");
	free(game->swap_png);
	mlx_image_to_window(game->mlx, game->player, game->move_y * 32, game->move_x * 32);
	return ;
}
...
void	ft_movement(mlx_key_data_t key_stroke, t_game *game)
{
	if (((key_stroke.key == MLX_KEY_D || key_stroke.key == MLX_KEY_RIGHT)
		&& key_stroke.action == MLX_PRESS) && (game->map[game->move_x][game->move_y + 1] != '1'))
		{
		game->orientation = 0;
		ft_player_right(game);
		ft_move_right(game);
	...
	if (((key_stroke.key == MLX_KEY_W || key_stroke.key == MLX_KEY_UP)
        && key_stroke.action == MLX_PRESS) && (game->map[game->move_x - 1][game->move_y] != '1'))
		ft_move_up(game);
